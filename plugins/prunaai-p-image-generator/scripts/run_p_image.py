#!/usr/bin/env python3
"""Generate an image with prunaai/p-image and save it locally."""
...
MODEL_OWNER = "prunaai"
MODEL_NAME = "p-image"
USER_AGENT = "prunaai-p-image-generator/0.1.0"
...
class PImageError(RuntimeError):
    """Raised for p-image wrapper failures."""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a single image with prunaai/p-image and save it locally."
    )
    parser.add_argument("prompt", help="Prompt passed directly to prunaai/p-image.")
    parser.add_argument(
        "--aspect-ratio",
        default="16:9",
        help="Aspect ratio passed to the model, such as 1:1, 16:9, or 9:16.",
    )
    parser.add_argument(
        "--output-name",
...
def api_request(
    ...
    try:
        ...
        raise PImageError(f"{method} {url} failed with HTTP {exc.code}: {message}") from exc
    except URLError as exc:
        raise PImageError(f"{method} {url} failed: {exc.reason}") from exc

    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise PImageError(f"{method} {url} returned non-JSON output.") from exc

    if not isinstance(parsed, dict):
        raise PImageError(f"{method} {url} returned an unexpected response shape.")
    return parsed
...
def resolve_version(token: str) -> str:
    ...
    if not isinstance(version, str) or not version:
        raise PImageError(
            "Could not resolve latest_version.id for prunaai/p-image."
        )
    return version
...
def sanitize_stem(value: str) -> str:
    ...
    return cleaned or "p-image-output"
...
def download_output(value: str) -> tuple[bytes, str]:
    ...
    try:
        ...
        message = exc.read().decode("utf-8", "replace")
        raise PImageError(f"Downloading output failed with HTTP {exc.code}: {message}") from exc
    except URLError as exc:
        raise PImageError(f"Downloading output failed: {exc.reason}") from exc
    ...
    if isinstance(output, list):
        for item in output:
            if isinstance(item, str) and item.startswith(URL_PREFIXES):
                return item
    raise PImageError("Prediction finished without a downloadable image output.")
